...
const SellerDetail = () => {
  ...
  useEffect(() => {
    if (!id) return;
    const fallback = sellerTestimonials.find((s) => s.id === id) || null;
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${base}/sellers/${id}`);
        if (!res.ok) throw new Error("Seller fetch failed");
        const data = (await res.json()) as SellerDetailItem;
        setSeller(data);
      } catch {
        setSeller(fallback);
      } finally {
  ...
  const story = seller.detail || {
    headline: seller.quote,
    summary: seller.outcome,
    heroImage: seller.image,
    highlights: [],
    metrics: [],
  };
  const heroImg =
            ...
            <div className="glass rounded-2xl p-6 border border-border/70 space-y-4">
              ...
              <Button asChild variant="hero" className="w-full">
                <Link to={story.ctaHref || "/contact"}>
                  {story.ctaLabel || "Plan a seller showcase"}
                </Link>
              </Button>
            </div>
          </div>
        ...
        <div className="container-custom space-y-8">
          ...
          <StickyScrollReveal
            ...
            onItemSelect={(item) => {
              ...
              if (target) navigate(`/sellers/${target}`);
            }}
          />
        </div>
      </section>
